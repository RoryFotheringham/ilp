...
public class StopSegment {
    ...
    private ArrayList<Path> possiblePaths(Graph graph){
        ...
        if(stores.size() == 1){
            Path path = PathFind.findPath(graph, this.stores.get(0), this.destination, this.getStops());
            paths.add(path);
            return paths;
        }else{
            ...
            Path path_01 = PathFind.findPath(graph, this.stores.get(1), this.destination, stops_01);
            ...
            Path path_10 = PathFind.findPath(graph, stores.get(1), stores.get(0), stops_10);
            ArrayList<Node> stops_11 = new ArrayList<>();
            stops_11.add(this.stores.get(0));
            stops_11.add(this.destination);
            Path path_11 = PathFind.findPath(graph, stores.get(0), destination, stops_11);
